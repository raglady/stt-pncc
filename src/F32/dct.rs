...
fn build_cos_table(n: usize) -> Vec<f32> {
    let mut table = vec![0.0_f32; n * n];
    let denom = 2.0 * n as f32;
    for k in 0..n {
        for j in 0..n {
            table[k * n + j] = ((2 * j + 1) as f32 * k as f32 * PI / denom).cos();
        }
    }
    table
}
...
fn dct_along_rows(input: &Array2<f32>, norm: Norm) -> Array2<f32> {
    ...
    for i in 0..rows {
        for v in 0..cols {
            ...
            output[[i, v]] = match norm {
                Norm::Ortho => {
                    if v == 0 {
                        (1.0 / cols as f32).sqrt() * sum
                    } else {
                        (2.0 / cols as f32).sqrt() * sum
                    }
                }
                Norm::None => 2.0 * sum,
            };
...
fn dct_along_columns(input: &Array2<f32>, norm: Norm) -> Array2<f32> {
...
pub fn dct_2d_axis(input: &Array2<f32>, norm: Norm, axis: Axis) -> Array2<f32> {
    match axis {
...
fn idct_along_rows(input: &Array2<f32>, norm: Norm) -> Array2<f32> {
    let (rows, cols) = input.dim();
    let cos = build_cos_table(cols);
    let mut output = Array2::<f32>::zeros((rows, cols));
    ...
    for i in 0..rows {
        for j in 0..cols {
            ...
            output[[i, j]] = match norm {
                Norm::Ortho => sum,
                Norm::None => sum / (2.0 * cols as f32), // ← divide by 2N, not N
            };
        }
    }
    output
...
fn idct_along_columns(input: &Array2<f32>, norm: Norm) -> Array2<f32> {
    let transposed = input.t().to_owned();
...
pub fn idct_2d_axis(input: &Array2<f32>, norm: Norm, axis: Axis) -> Array2<f32> {
    match axis {
...
mod tests {
    ...
    fn dc_coefficient_none() {
        ...
        let expected_dc = 2.0 * x.iter().sum::<f32>(); // 2 · Σ x[n]
        assert!(
