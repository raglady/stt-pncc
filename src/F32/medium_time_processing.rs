...
///
pub fn medium_time_power_calculation(power_stft_signal: &Array2<f32>, m: usize) -> Array2<f32> {
    let (rows, cols) = power_stft_signal.dim();
    let mut medium_time_power = Array2::<f32>::zeros([rows, cols]);

    // Create padded array
    let padded_rows = rows + 2 * m;
    let mut padded = Array2::<f32>::zeros([padded_rows, cols]);
    ...
    for i in 0..rows {
        for j in 0..cols {
            ...
            medium_time_power[[i, j]] = sum / (2 * m + 1) as f32;
        }
    }
...
pub fn asymmetric_lowpass_filtering(
    rectified_signal: &Array2<f32>,
    lm_a: f32,
    lm_b: f32,
) -> Array2<f32> {
    let (rows, cols) = rectified_signal.dim();
    let mut floor_level = Array2::<f32>::zeros([rows, cols]);
...
pub fn temporal_masking(rectified_signal: &Array2<f32>, lam_t: f32, myu_t: f32) -> Array2<f32> {
    let (rows, cols) = rectified_signal.dim();
    let mut temporal_masked_signal = Array2::<f32>::zeros([rows, cols]);
    let mut online_peak_power = Array2::<f32>::zeros([rows, cols]);
...
pub fn switch_excitation_or_non_excitation(
    temporal_masked_signal: &Array2<f32>,
    floor_level: &Array2<f32>,
    lower_envelope: &Array2<f32>,
    medium_time_power: &Array2<f32>,
    c: f32,
) -> Array2<f32> {
...
pub fn weight_smoothing(
    ...
    let mut spectral_weight_smoothing = Array2::<f32>::zeros([rows, cols]);
    ...
    spectral_weight_smoothing
        ...
        .for_each(|((m, l_idx), val)| {
            ...
            for l_ in l_1..l_2 {
                ...
                sum += final_output[[m, l_]] / medium_time_power[[m, l_]].max(f32::EPSILON);
            }

            *val = sum / (l_2 as isize - l_1 as isize + 1) as f32;
        });
