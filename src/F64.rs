...
impl<'s> PNCCArgs<'s> {
    ...
    pub fn get_samples_rate(&self) -> f64 {
        self.samples_rate
    }

    pub fn get_win_hop() -> f64 {
        0.010
    }

    pub fn get_win_len() -> f64 {
        0.025
    }
}
...
pub fn deltas(ceps: &Array2<f64>, win: usize) -> Array2<f64> {
    ...
    let norm: f64 = 2.0 * (1..=h).map(|k| (k * k) as f64).sum::<f64>();
...
pub fn pncc(args: &PNCCArgs) -> Array2<f64> {
    let weight_n = 4;
    ...
    let stft_powerd = stft_ed
        .map(|x| (1.0 / args.get_nfft() as f64) * x.powi(args.get_power().try_into().unwrap()));
...
mod tests {
    ...
    fn test_shape_preserved() {
        let ceps = Array2::<f64>::zeros((10, 13));
        let d = deltas(&ceps, 5);
    ...
    fn test_edge_cases() {
        ...
        // Matrice vide
        let empty = Array2::<f64>::zeros((0, 13));
    ...
    fn test_linear_ramp_has_constant_delta() {
        ...
        let n = 20usize;
        let ceps = Array2::from_shape_fn((n, 1), |(f, _)| f as f64);
        let d = deltas(&ceps, 5);
