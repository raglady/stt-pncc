...
// ERB Constant
pub const EAR_Q: f64 = 9.26449;
pub const MIN_BW: f64 = 24.7;
...
pub fn erb_space(low_freq: f64, high_freq: f64, n: u8) -> Vec<f64> {
    // All of the following expressions are derived in Apple TR #35, "An
    // Efficient Implementation of the Patterson-Holdsworth Cochlear
    // Filter Bank." See pages 33-34.
    let vec: Vec<f64> = (1..=n)
        .map(|i| {
            let exponent = ((i as f64)
...
pub fn erb_filter_bank(
    ...
    let mut feedback = Array2::<f64>::zeros([n_filts.into(), 9]);
    let mut forward = Array2::<f64>::zeros([n_filts.into(), 5]);
    let mut output = Array2::<f64>::zeros([n_filts.into(), input_puls.len()]);
    ...
    for i in 0..n_filts as usize {
        ...
        let gain_tmp = ((-2.0 * (Complex64::new(0., 4.0 * cf[i] * PI * t)).exp() * t
            + 2.0
                * Complex64::new(-(temp_b * t), 2.0 * cf[i] * PI * t).exp()
                * t
                * ((2.0 * cf[i] * PI * t).cos()
                    - (3.0 - (2.0f64).powf(1.5)).sqrt() * (2.0 * cf[i] * PI * t).sin()))
            * (-2.0 * Complex64::new(0., 4.0 * cf[i] * PI * t).exp() * t
                + 2.0
                    * Complex64::new(-(temp_b * t), 2.0 * cf[i] * PI * t).exp()
                    * t
                    * ((2.0 * cf[i] * PI * t).cos()
            ...
            * (-2.0 * Complex64::new(0., 4.0 * cf[i] * PI * t).exp() * t
            ...
            / (-2.0 / (2.0 * temp_b * t).exp()
                - 2.0 * Complex64::new(0., 4.0 * cf[i] * PI * t).exp()
                + 2.0 * (1.0 + Complex64::new(0., 4.0 * cf[i] * PI * t).exp())
                    / (temp_b * t).exp())
            .powf(4.0))
        .abs();
        gain[i] = gain_tmp;
...
mod tests {
    ...
    fn test_erb_space_custom() {
        ...
        assert!(cf_array[49] > 100.0 && cf_array[49] < 200.0);
