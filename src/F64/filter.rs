...
pub fn filter(numerator: &[f64], denominator: &[f64], signal: &[f64]) -> Vec<f64> {
    // --- Validation ---
    assert!(
    ...
    let b: Vec<f64> = numerator.iter().map(|&v| v / a0).collect();
    let a: Vec<f64> = denominator.iter().skip(1).map(|&v| v / a0).collect();
...
mod tests {
    ...
    fn test_iir_leaky_integrator() {
        ...
        let y = filter(&b, &a, &x);
        // y[n] = 0.9^n
        let expected: Vec<f64> = (0..5).map(|n| 0.9_f64.powi(n)).collect();
