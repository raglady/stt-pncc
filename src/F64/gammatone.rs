...
pub fn fft2gammatonemx(
    ...
    let ucirc: Vec<Complex64> = (0..=nfft / 2)
        .map(|i| Complex64::new(0., 2.0 * PI * i as f64 / nfft as f64).exp())
        .collect();

    let justpoles = 0;
    let mut gain = vec![0.; n_filts.into()];
    let mut wts = Array2::<f64>::zeros([n_filts.into(), nfft / 2 + 1]);
    ...
    for i in 0..n_filts as usize {
        ...
        if justpoles == 1 {
            ...
            let center = Complex64::new(0., omegamax).exp();
            ...
            let a11 = -(2.0 * t * (2.0 * cf * PI * t).cos() / (b_tmp * t).exp()
                + 2.0 * (3.0 + (2.0f64).powf(1.5)).sqrt() * t * (2.0 * cf * PI * t).sin()
                    / (b_tmp * t).exp())
                / 2.0;
            ...
            let zros: Vec<f64> = [a11, a12, a13, a14].iter().map(|x| -x / t).collect();

            gain[i] = ((-2.0 * Complex64::new(0., 4.0 * cf * PI * t).exp() * t
                + 2.0
                    * Complex64::new(-(b_tmp * t), 2.0 * cf * PI * t).exp()
                    * t
                    * ((2.0 * cf * PI * t).cos()
                        - (3.0 - (2.0f64).powf(1.5)).sqrt() * (2.0 * cf * PI * t).sin()))
                * (-2.0 * Complex64::new(0., 4.0 * cf * PI * t).exp() * t
                    + 2.0
                        * Complex64::new(-(b_tmp * t), 2.0 * cf * PI * t).exp()
                        * t
                        * ((2.0 * cf * PI * t).cos()
                ...
                * (-2.0 * Complex64::new(0., 4.0 * cf * PI * t).exp() * t
                    + 2.0
                        ...
                        * ((2.0 * cf * PI * t).cos()
                            + (3.0 + (2.0f64).powf(1.5)).sqrt() * (2.0 * cf * PI * t).sin()))
                / (-2.0 / (2.0 * b_tmp * t).exp()
                    - 2.0 * Complex64::new(0., 4.0 * cf * PI * t).exp()
                    + 2.0 * (1.0 + Complex64::new(0., 4.0 * cf * PI * t).exp())
                        / (b_tmp * t).exp())
                .powf(4.0))
            .abs();
            wts.row_mut(i).assign(&aview1(
        ...
        let value = {
            ...
            row.iter().map(|c| c / max).collect::<Vec<f64>>()
        };
